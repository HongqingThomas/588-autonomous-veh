...
class PurePursuit(object):
    ...
    def read_waypoints(self):
        ...
        filename = os.path.join(dirname, '/home/gem/demo_ws/src/vehicle_drivers/gem_gnss_control_/waypoints/smoothpath.csv')
    ...
    def start_pp(self):
        ...
        while not rospy.is_shutdown():
            ...
            print("curr_x, curr_y, curr_yaw", curr_x, curr_y, curr_yaw)

            # # finding the distance of each way point from the current position
            for i in range(len(self.path_points_x)):
                self.dist_arr[i] = self.dist((self.path_points_x[i], self.path_points_y[i]), (curr_x, curr_y))

            # self.dist_arr = np.ones(len(self.path_points_x)) * 10000
            # if self.prev_idx + 199 < len(self.path_points_x):
            #     for i in range(self.prev_idx, self.prev_idx + 199):
            #         self.dist_arr[i] = self.dist((self.path_points_x[i], self.path_points_y[i]), (curr_x, curr_y))
            # else:
            #     for i in range(self.prev_idx, len(self.path_points_x)):
            #         self.dist_arr[i] = self.dist((self.path_points_x[i], self.path_points_y[i]), (curr_x, curr_y))
            ...
            # print("self.path_points_heading[self.goal] - curr_yaw", self.path_points_heading[self.goal], curr_yaw)
            ...
            # self.turn_pub.publish(self.turn_cmd)
            # print("self.accel_cmd", self.accel_cmd.f64_cmd)
            if self.goal < 400:
                self.accel_cmd.f64_cmd = output_accel
                self.steer_cmd.angular_position = np.radians(steering_angle)
                self.accel_pub.publish(self.accel_cmd)
                ...
                self.brake_cmd.f64_cmd = 0.45
                self.steer_cmd.angular_position = np.radians(0.0)
                self.brake_pub.publish(self.brake_cmd)
                self.steer_pub.publish(self.steer_cmd)
                self.turn_pub.publish(self.turn_cmd)
                print("self.brake_cmd", self.brake_cmd.f64_cmd)
